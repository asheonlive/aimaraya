...
import {
  ...
  Sparkles, PanelLeft, LogOut, MessageCircle, Bell,
} from "lucide-react";

const CREATE_NAV = [
  { to: "/app/create-image", label: "Image", icon: ImageIcon },
  { to: "/app/create-video", label: "Video", icon: Video },
  { to: "/app/edit-image", label: "Edit Image", icon: Pencil },
  { to: "/app/edit-3d", label: "Edit 3D", icon: Box },
  { to: "/app/video-editor", label: "Edit Video", icon: Film, beta: true },
  { to: "/app/background-change", label: "BG Change", icon: Wand2 },
  { to: "/app/moodboard", label: "Moodboard", icon: LayoutGrid, beta: true },
];
const ASSET_NAV = [
                  ...
                  <NavLink to={it.to} data-testid={`side-${it.label.toLowerCase().replace(/\s+/g,"-")}`} className={({ isActive }) => `${isActive ? "bg-white/[0.08] text-white font-medium" : "text-white/70 hover:bg-white/[0.05] hover:text-white"} group/menu-item relative flex items-center gap-2 rounded-md px-2 py-2 text-sm`}>
                    {({ isActive }) => (
                      <>
                        ...
                        {!collapsed && it.beta && <span className="ml-auto bg-amber-600 text-white text-[10px] font-semibold uppercase leading-none rounded-full px-1.5 py-0.5">BETA</span>}
                      </>
                    )}
                  </NavLink>
                </li>
