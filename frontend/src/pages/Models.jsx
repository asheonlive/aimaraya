...
export default function Models() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16">
      ...
      <p className="text-[#a89dc9] max-w-xl mb-10">Every frontier model in one studio. Videos use your daily allowance.</p>
        ...
        {filtered.map(m => (
          <div key={m.id} data-testid={`model-card-${m.id}`} className={`card-purple p-6 hover-lift ${m.available ? "" : "opacity-70"}`}>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-[#2a2340]">
              <span className="font-mono text-sm">{m.type === "video" ? "Daily video" : "Included"}</span>
              {m.available
